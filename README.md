...
TwiOpSec, Paper ve Folia sunucularında operatör ve yüksek ayrıcalıklı izinleri UUID tabanlı beyaz listelerle koruyan, tek JAR olarak dağıtılan bağımsız bir güvenlik eklentisidir. T2C-OPSecurity yapılandırmalarını ilk açılışta işlemsel olarak içe aktarır; T2CodeLib çalışma zamanı bağımlılığı yoktur.
...
- Bilinen PlugMan/PlugManX biçimleriyle TwiOpSec'i runtime'da unload/reload etme ve Paper/Bukkit `/reload` denemeleri engellenir.
...
1. Sunucuyu durdurun ve `plugins/TwiOpSec-1.0.0.jar` dosyasını yerleştirin.
...
Oyuncu göndericinin hem `twiopsec.admin` iznine hem de en az bir TwiOpSec güven listesi kaydına sahip olması gerekir. Konsol ve RCON yönetim komutlarını çalıştırabilir. Ayarlar ve örnek açıklamalar [`config.yml`](src/main/resources/config.yml) içindedir.
...
Derleme çıktısı `build/libs/TwiOpSec-1.0.0.jar` olur. Projede CI/CD tanımı bilinçli olarak yoktur; doğrulama ve yayın yerel kalite kapılarıyla yapılır.

## Sınırlar ve kaynaklar

Aynı JVM'de çalışan kötü niyetli bir eklenti güvenlik sınırının içindedir: Bukkit API'sini doğrudan çağırabilir, listener'ları kaldırabilir, TwiOpSec'i disable edebilir veya dosyalara yazabilir. Komut engeli ve disable kanıtı savunma katmanıdır, mutlak engel değildir. Eklenti allowlist'i, dosya izinleri, ayrı servis hesabı, çevrimiçi kimlik doğrulama, güncel Paper/Folia ve yalnızca güvenilir JAR'lar kullanın.
